 ...
 *
 * The Statement Prototype generates a prototype for a statement bind
 ...
 * This macro generates the front matter for the prototype for a function
 * which binds each of the fields in the schema to a SQLite3 statement object
 * which has been prepared with indexed fields.
 * @a _STRUCTURE_ is the schema name.
 */
#define PLACER_SCHEMA(_STRUCTURE_) \
extern int placer_struct_##_STRUCTURE_##_stmt_bind(sqlite3_stmt * sp, const struct _STRUCTURE_ * pp)
...
/**
 * @def PLACER_SCHEMA(_STRUCTURE_)
 * This macro generates the end matter for the prototype for a function
 * which binds each of the fields in the schema to a SQLite3 statement object
 * which has been prepared with indexed fields.
 * @a _STRUCTURE_ is the schema name.
 */
