...
The functions generated by the x macros check the field name, field data
type, and the table name (where possible) to verify that they match the
schema they were compiled against. If the function finds a conflict,
it immediately returns SQLITE_ABORT (whether the function was a callback
for a query or not, regardless of what the error message says). However,
there are lots of reasons for the code where these checks are performed
